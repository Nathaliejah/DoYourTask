...
export default function App() {
  ...
  const [completedNotes, setCompletedNotes] = useState([]); // Added for completed notes
  ...
  const [viewCompleted, setViewCompleted] = useState(false); // Updated for Notes
  ...
  const currentViewItems = viewDeleted
    ...
    ? activeTab === 'Tasks'
      ? completedTasks
      : completedNotes // Updated for Notes
    : filteredItems;
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        placeholder={`Search ${activeTab.slice(0, -1)}`} // Singular form
        value={searchQuery}
...
const styles = StyleSheet.create({
  ...
  footer: {
    position: 'absolute',   // Ensures it's at the bottom
    bottom: 20,             // Controls how far from the bottom it should be
    fontSize: 12,
    color: '#aaa',
  },  
});
